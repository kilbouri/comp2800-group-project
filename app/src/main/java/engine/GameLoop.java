...
public abstract class GameLoop extends Canvas implements Runnable {
  ...
  private void doUpdate(final double deltaTime) {
    ...
    GameObject[] currentObjects = gameObjects.toArray(
      new GameObject[gameObjects.size()]
    );
    ...
    for (int i = 0; i < currentObjects.length; i++) {
      ...
      BoxCollider collision =
        currentObjects[i].getComponent(BoxCollider.class);
      if (collision != null) {
  ...
  protected void renderEngineMetrics(Graphics2D graphics) {
    double averageFps =
      lastFpsCounts.stream().reduce(0.0, Double::sum) / lastFpsCounts.size();
